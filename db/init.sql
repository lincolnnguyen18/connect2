...
CREATE PROCEDURE get_messages_offset(_sender_id INT, _recipient_id INT, _limit INT, _offset INT) BEGIN
  SELECT messages.body, messages.created_at, IF(messages.sender_id = _sender_id, 'sender', 'recipient') AS direction FROM messages WHERE
    ((messages.sender_id = _sender_id AND messages.recipient_id = _recipient_id) OR
    (messages.sender_id = _recipient_id AND messages.recipient_id = _sender_id))
    AND messages.id < _offset
    ORDER BY messages.id DESC
    LIMIT _limit;
END //

CREATE PROCEDURE get_messages(_sender_id INT, _recipient_id INT, _limit INT) BEGIN
  SELECT messages.body, messages.created_at, IF(messages.sender_id = _sender_id, 'sender', 'recipient') AS direction FROM messages WHERE
    (messages.sender_id = _sender_id AND messages.recipient_id = _recipient_id) OR
    (messages.sender_id = _recipient_id AND messages.recipient_id = _sender_id)
    ORDER BY messages.id DESC
